# carta_bola_de_fogo.gd
extends Card

func _ready():
	carta_nome = "Bola de Fogo"
	carta_tipo = 2
	carta_valor = 15
	carta_descricao = "Lança 2 bolas de fogo que perseguem inimigos diferentes"
	super._ready()
...
func buscar_tres_inimigos_proximos(personagem: Node3D) -> Array:
	...
	var alvos = []
	var max_alvos = min(2, inimigos_com_distancia.size())
...
func spawnar_bolas_sequencial(personagem: Node3D, alvos: Array):
	for i in range(alvos.size()):
		...
		# Verificar se alvo ainda existe antes de usar
...
func spawnar_bola_unica(personagem: Node3D, alvo: Node3D, indice: int):
	...
	var efeito = preload("res://cenas/cartas/carta_bola_de_fogo/efeito_bola_de_fogo.tscn")
